...
const FlightPass = ({ 
  ...
  return (
    <div 
      ...
      className="bg-white rounded-3xl shadow-washi border border-stone-200 overflow-hidden relative mb-6 transition-transform hover:scale-[1.01] duration-300 cursor-pointer active:scale-[0.98] select-none"
    >
      ...
      <div className="p-6 pt-8 relative z-10">
         ...
         <div className="flex items-center justify-between mb-8">
           ...
           <div className="flex-1 px-4 flex flex-col items-center">
             {/* Animated Plane Icon */}
             <div className={`text-wafu-gold mb-1 ${isReturn ? 'rotate-[185deg]' : '-rotate-5'} ${isFlying ? 'animate-fly-grand' : 'animate-fly'} origin-center`}>
                <Icons.Plane />
             </div>
...
export default function App() {
  ...
  useEffect(() => {
    // 1. Itinerary
    const unsubItinerary = onSnapshot(query(collection(db, 'itinerary')), (snapshot) => {
  ...
  return (
    <div className="max-w-md mx-auto h-[100dvh] bg-wafu-paper relative flex flex-col shadow-2xl overflow-hidden font-sans text-base">
      ...
      {/* Falling Sakura Petals Overlay with Backdrop Blur */}
      {sakuraPetals.map(petal => (
        <div
          key={petal.id}
          className={`fixed z-50 pointer-events-none animate-sakura-fall ${petal.color} backdrop-blur-[1px]`}
          style={{
            left: petal.left,
            width: petal.size,
            height: petal.size,
      ...
      {/* Header - Luxury Sakura */}
      <div className="relative z-30 pt-14 pb-4 px-6 flex justify-between items-center bg-wafu-bg/90 backdrop-blur-md border-b border-wafu-indigo/5 shadow-sm shrink-0">
        ...
        <div className="flex items-center gap-4 cursor-pointer active-bounce" onClick={triggerSakura}>
           {/* Sakura Crest - Indigo Blue */}
           <div className={`w-12 h-12 relative shrink-0 text-wafu-indigo ${isSpinning ? 'animate-jump-spin' : ''}`}>
              <Icons.Sakura />
           </div>
           <div>
             <h1 className="text-[26px] font-serif font-black text-wafu-indigo tracking-[0.2em] leading-tight text-gold-leaf">
               京都八日遊
             </h1>
             <p className="text-[8px] text-wafu-gold font-bold tracking-[0.4em] uppercase mt-0.5 opacity-80">Kyoto Journey</p>
           </div>
        </div>
        <a 
          ...
          target="_blank" 
          rel="noreferrer"
          className="group relative flex flex-col items-center justify-center w-9 h-9 bg-wafu-indigo text-white rounded-xl shadow-md hover:bg-wafu-darkIndigo transition-all duration-100 active-bounce overflow-hidden border border-wafu-indigo"
        >
          <Icons.QrCode />
          <div className="absolute bottom-0.5 right-0.5 w-1.5 h-1.5 bg-wafu-gold rounded-full shadow-sm"></div>
        </a>
      </div>
      ...
      >
        <div key={activeTab} className="animate-fade-in-up min-h-full flex flex-col">
          {activeTab === 'itinerary' && (
            <>
              ...
              <div className="flex-1 pt-6 pb-32 bg-seigaiha bg-fixed bg-top">
                ...
                <div key={selectedDay} className="animate-fade-in-up">
                    <Itinerary 
