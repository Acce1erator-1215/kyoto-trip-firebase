...
export const SightseeingList: React.FC<Props> = ({ items }) => {
  ...
  const openAdd = () => {
    setEditingId(null);
    setForm({ name: '', description: '', imageUrl: '', mapsUrl: '' });
    setIsAdding(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
        if (editingId) {
            await updateDoc(doc(db, 'sightseeing', editingId), { ...form });
        } else {
            const newId = Date.now().toString();
            await setDoc(doc(db, 'sightseeing', newId), {
                id: newId,
                name: form.name,
    ...
    <div className="pb-40 px-5">
      ...
      <div className="space-y-6">
        {activeItems.map(item => (
           <div key={item.id} className="bg-white rounded-2xl shadow-washi border border-stone-100 overflow-hidden flex flex-col sm:flex-row group transition-all hover:shadow-luxury relative">
        ...
        {deletedItems.length > 0 && (
          <div className="mt-8 px-2">
             ...
             >
                <Icons.Trash />
                <span>已刪除景點 ({deletedItems.length})</span>
             </button>
        ...
        <div className="fixed inset-0 bg-wafu-darkIndigo/60 backdrop-blur-sm z-[9999] flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-2xl shadow-2xl animate-in slide-in-from-bottom-10 duration-300 relative max-h-[85dvh] flex flex-col">
             ...
             <div className="shrink-0 px-6 py-4 flex items-center justify-between border-b border-stone-100 bg-stone-50/50 rounded-t-2xl">
                 ...
                 <h3 className="text-lg font-bold font-serif text-wafu-indigo tracking-widest">
                    {editingId ? '編輯景點' : '新增景點'}
                 </h3>
                 <button 
                    onClick={handleSave}
