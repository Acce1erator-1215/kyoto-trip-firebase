...
export const ShoppingList: React.FC<Props> = ({ items, expenses }) => {
  ...
  const handleDelete = async (id: string, item: ShoppingItem) => {
      try {
        ...
        // Or keep it? The requirement said "delete expense when cancelling purchase", 
        // so if we delete the item entirely, the expense should probably go too.
    ...
    <div className="pb-40 px-5">
      ...
      <div className="grid grid-cols-2 gap-4 mb-10">
        {activeItems.map(item => {
           ...
           return (
            <div key={item.id} className="bg-white rounded-2xl shadow-washi border border-stone-100 overflow-hidden group flex flex-col relative transition-transform duration-300 hover:-translate-y-1 hover:shadow-lg active:scale-[0.98]">
                ...
                <button 
                    onClick={() => toggleBought(item.id, item)}
                ...
                {item.bought && (
                    <div className="absolute inset-0 z-30 pointer-events-none flex items-center justify-center bg-white/10 backdrop-blur-[1px]">
      ...
      {isAdding && (
        <div className="fixed inset-0 bg-wafu-darkIndigo/60 backdrop-blur-sm z-[9999] flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-2xl shadow-2xl animate-in slide-in-from-bottom-10 duration-300 relative max-h-[85dvh] flex flex-col">
             
             <div className="shrink-0 px-6 py-4 flex items-center justify-between border-b border-stone-100 bg-stone-50/50 rounded-t-2xl">
                 <button 
                 ...
                 <h3 className="text-lg font-bold font-serif text-wafu-indigo tracking-widest">伴手禮追加</h3>
                 <button 
                    onClick={handleAdd}
             ...
             <div className="flex-1 overflow-y-auto px-8 py-8 pb-32 relative">
